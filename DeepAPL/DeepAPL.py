    ...
    def Import_Data(self,directory,Load_Prev_Data=False,classes=None,sample=None,
        ...
        if Load_Prev_Data is False:
            ...
            #        }
                ...
                ##Not sure what is the intention of below code especially directory + d:
                #start
                classes = [d for d in os.listdir(directory) if os.path.isdir(directory + d)]
                #print("This is Classes d for d");#print(classes); --np
                classes = [f for f in classes if not f.startswith('.')]
                
                #print("This is Classes f for f") --np
                #print(classes); --np
                #End

            if not classes:
                classes = [directory.split('/')[-1]]
                #classes =[directory_value] #nothing much happening here
                no_folders = True
                
            #Label encoder transform categorical labels into unique integers
            # as of this step classes value is nothing but directory value
            self.lb = LabelEncoder()
            
            #Fit the encoder to your categorical labels - fit function is used for adding array to the label encoder
            self.lb.fit(classes)
            ...
            if save_data is True:
                np.save(os.path.join(self.Name, 'imgs'), imgs) ##All the images are stored in npy file in an array format.
                with open(os.path.join(self.Name, 'data.pkl'), 'wb') as f:
                    pickle.dump([Y,labels,patients,cell_type,files,smears,self.lb], f, protocol=4) ##data pickle files has meta data information about the image files. 
...
class DeepAPL_SC(base):
        ...
        with tf.device(self.device):
            ...
            with GO.graph_model.as_default():
                ...
                for _ in range(num_fc_layers):
                    fc = tf.compat.v1.layers.dense(fc,12,tf.nn.relu)
                if multisample_dropout_num_masks is not None:
                    ...
                    GO.logits = tf.compat.v1.layers.dense(fc, GO.Y.shape[1])
                ...
                GO.loss = tf.reduce_mean(weights*tf.compat.v1.nn.softmax_cross_entropy_with_logits_v2(GO.Y,GO.logits))

                GO.opt = tf.compat.v1.train.AdamOptimizer(learning_rate=0.001).minimize(GO.loss)
    ...
    def _train(self,batch_size = 10, epochs_min = 10,stop_criterion=0.001,stop_criterion_window=10,
        ...
        with tf.compat.v1.Session(graph=GO.graph_model, config=config) as sess:
...
class DeepAPL_WF(base):
    ...
    def _build(self,weight_by_class=False,multisample_dropout_num_masks = None,graph_seed=None,
        ...
        with tf.device(self.device):
            ...
            with GO.graph_model.as_default():
                ...
                GO.sp = tf.compat.v1.sparse_placeholder(dtype=tf.float32, shape=[None, None], name='sp') #Niteesh added
                Features = Conv_Model(GO,l1_units=l1_units,l2_units=l2_units,l3_units=l3_units)
                GO.w = Features
                GO.w = tf.identity(GO.w,'w')
                fc =  tf.reduce_max(GO.w, [1, 2])
                ...
                for _ in range(num_fc_layers):
                    fc = tf.compat.v1.layers.dense(fc,12,tf.nn.relu)
                fc = tf.compat.v1.layers.dense(fc,GO.Y.shape[1])
                ...
                GO.logits = tf.sparse.sparse_dense_matmul(GO.sp, fc)/sum
    ...
    def Inference(self,model='model_0',batch_size=100):
        ...
        with tf.compat.v1.Session(config=config) as sess:
            saver = tf.compat.v1.train.import_meta_graph(os.path.join(self.Name, 'models',model, 'model.ckpt.meta'))
            saver.restore(sess, tf.train.latest_checkpoint(os.path.join(self.Name,'models', model)))
