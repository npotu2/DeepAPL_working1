...
from DeepAPL.functions.color_norm import * #Niteesh changed this - original DeepAPL.functions.color_norm
import copy
...
def Get_Images(sub_dir,sample,include_cell_types=None,exclude_cell_types=None,load_images=True,color_norm=True,nmf=False,rm_rbc=False):
    ...
    #sub_dir value would be C:\Users\npotu1\DataScience\DeepAPL\Data\All\Patient_00
    #below step would add Signed slides to sub_dir --> C:\Users\npotu1\DataScience\DeepAPL\Data\All\Patient_00\Signed slides
    sub_dir_2 = os.path.join(sub_dir,'Signed slides')
    
    #type_list variable stores list of all directories or files in sub_dir_2, in current context it inludes types of slides
    type_list = os.listdir(sub_dir_2)
    
    #The Get_Images function has default values as "None" for include_cell_types and exclude_cell_types
    #Not sure why these If conditions were written its never going to be "not None" when it was hard coded as "None"
    ...
    for type in type_list:
        #example type == one of the folder in signed slides folder 
        #files_read variable stores the list of paths of all images in type folder.
        files_read = glob.glob(os.path.join(sub_dir_2, type, '*jpg'))
        
        # loop through each file in the folder
        for file in files_read:
            #appending all files in an array
            files.append(file)
            
            cell_type.append(type)
            
            if load_images is True:
                #The imread function is commonly used in computer vision and image processing libraries to read an image from a file. 
                # In Python, the OpenCV library provides this functionality.
                img = imread(file)
                #resizing the image 
    ...
    #Sample value was assigned None in Import_Data function. 
    ...
    if len(imgs) != 0:
        #Default value of color_norm is true 
        if color_norm:
            # normalize imgs
            cns = ColorNormStains()
            cns.process_img_data(imgs)
            imgs_norm,imgs_nmf = cns.Get_Normed_Data()
            if rm_rbc:  #default value False
                #get masks
